...
class NARDAcontroller():
    ...
    def read_one_mode(self, mode, start, stop, step):
        # time.sleep(2)
        self.port.flushInput()
        self.port.flushOutput()
    ...
    def read_data(self):
        ...
        start_freq = 13000
        self.port.write(str.encode('#00(i' + str(start_freq) + '*\r'))
        out = self.port.readline()
        ...
        step_freq = 1000
        self.port.write(str.encode('#00(s' + str(step_freq) + '*\r'))
        out = self.port.readline()
        ...
        self.port.write(str.encode('#00(f' + str(stop_freq) + '*\r'))
        out = self.port.readline()
        ...
        for i in range(3, 6):
            self.read_one_mode(i, start_freq, stop_freq+step_freq, step_freq)
            self.port.flush()
            self.port.flushOutput()
            self.port.flushInput()
