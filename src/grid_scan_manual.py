...
def convert_to_pts(dist, arr, x_off=0, y_off=0):
    x_dim = len(arr)
    y_dim = len(arr[0])
    xpts = []
    ypts = []
...
def run_scan(args):
    ...
    while True:
        # Plot results
        if args.measure:
            X, Y, Z = convert_to_pts(args.grid_step_dist, values)
            if place is not None:
                x_off = (place[0] - values.shape[0]/2) * args.grid_step_dist
                y_off = (place[1] - values.shape[1]/2) * args.grid_step_dist
                zoomX, zoomY, zoomZ = convert_to_pts(args.grid_step_dist/4, zoom_pts, x_off, y_off)
                print zoomX
                print zoomY
                print zoomZ
                X.extend(zoomX)
